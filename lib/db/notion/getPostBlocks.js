...
function applyHtmlEmbedBlocks(blockMap) {
  ...
  const createHtmlEmbedBlock = (html, templateBlock, parentId) => {
    if (!html || !html.trim()) return null
    let htmlBlockId = `${templateBlock?.id || parentId}_html_embed`
    let suffix = 1
    while (blockMap[htmlBlockId]) {
      htmlBlockId = `${templateBlock?.id || parentId}_html_embed_${suffix}`
      suffix += 1
    }
