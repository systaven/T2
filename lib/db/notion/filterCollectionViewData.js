function filterCollectionViewData(blockMap) {
  ...
  Object.values(blockMap.collection_view).forEach(entry => {
    ...
    if (sorts.length > 0) {
      const sorter = (leftId, rightId) => {
        const leftBlock = blockMap.block?.[leftId]?.value || blockMap.block?.[leftId]
        const rightBlock = blockMap.block?.[rightId]?.value || blockMap.block?.[rightId]
        return compareCollectionBlocks(leftBlock, rightBlock, sorts, schema)
      }
      sortBlockIdsInPlace(viewQuery, sorter)
      if (Array.isArray(view.page_sort)) {
        view.page_sort.sort(sorter)
      }
    }
  })
}

function sortBlockIdsInPlace(value, sorter) {
  if (!value || typeof value !== 'object') return

  if (Array.isArray(value.blockIds)) {
    value.blockIds.sort(sorter)
  }

  Object.values(value).forEach(child => sortBlockIdsInPlace(child, sorter))
}
