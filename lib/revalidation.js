...
export async function buildSiteContentSignature() {
  ...
  const normalized = snapshots.map(({ locale, props }) => ({
    ...
    siteInfo: {
      ...
      pageCover: normalizeSignatureValue(props?.siteInfo?.pageCover),
      icon: normalizeSignatureValue(props?.siteInfo?.icon)
    },
    pages: (props?.allPages || [])
      .map(page => ({
        ...
        pageCover: normalizeSignatureValue(page?.pageCover),
        pageCoverThumbnail: normalizeSignatureValue(page?.pageCoverThumbnail)
      }))
...
function normalizeSignatureValue(value) {
  ...
  try {
    ...
    if (!isVolatileNotionUrl(url)) {
      return value
    }

    url.searchParams.delete('expirationTimestamp')
...
function isVolatileNotionUrl(url) {
  return (
    url.hostname === 'file.notion.com' ||
